...
def chon_hieu_ung(tong_giay: float, muc: str = "vua",
    ...
    # GỘP 09/08/2026: giữ CẢ HAI vế của 2 nhánh — điều kiện nhường chỗ của nhánh
    # LỚP PHỦ **và** tham số `xoay` của nhánh MỞ RỘNG KHO. `da_dung` (không phải
    # `[]`) vì lúc này `dat_truoc` có thể đã đặt một kiểu rồi, hook không được
    # trùng kiểu với nó.
    if hook and float(tong_giay) >= 3.0 and len(ra) < n_diem \
            and not any(abs(HOOK_BAT - float(r["bat"])) < CACH_MIN for r in ra):
        k_hook = _chon_kieu("hook", dung, da_dung, 0, xoay)
        if k_hook:
