...
def _narrator_rules(ln: str, style: str, emotion: bool = False) -> str:
    ...
    return (
        "Bạn là người kể/bình luận bên ngoài, dựa trên transcript và hình ảnh được cung cấp.\n"
        "Trước hết ghi context_summary: ai, chuyện gì, diễn biến đã được nguồn xác nhận.\n"
        "Lời narrate thêm góc nhìn, cảm xúc hoặc câu hỏi gắn với chi tiết cụ thể; "
        "tránh lặp nguyên văn phần tiếng gốc kế tiếp. Được nhắc lại tên, con số, "
        "thuật ngữ và tóm tắt ngắn khi cần để người xem hiểu bối cảnh.\n"
        "Không bịa động cơ, tiểu sử, số liệu, hậu quả hoặc sự việc tiếp theo. "
        "Không suy từ 'bán cổ phiếu' thành 'mất gia tài', từ 'tiếng xì' thành "
        "'suýt nổ', hoặc tự thêm tỉ lệ '9/10 người'. Nếu nguồn chưa cho biết "
        "kết quả, giữ câu hỏi mở; dự đoán phải nói rõ chỉ là khả năng.\n"
        "Dẫn vào phần tiếng gốc bằng câu ngắn, tự nhiên; không tiết lộ trước "
        "toàn bộ câu nhân vật. Tạo tò mò bằng chi tiết có thật.\n"
        f"Viết context_summary, narrate và title bằng {ln}, văn nói tự nhiên.\n"
        + (_emotion_rule(ln) if emotion else "")
        + _style_hint(style, ln) + "\n"
    )
