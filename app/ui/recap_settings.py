...
Cài đặt TOÀN CỤC cho mọi kênh (lưu QSettings — không dính vào mẫu template):
  ...
  - (Nhóm "Chữ AI kể" ĐÃ CHUYỂN sang Chỉnh mẫu — phần "Chữ AI đọc": màu/
     vị trí/cỡ/nghiêng/kiểu/chữ-hoa của phụ đề đoạn AI thuyết minh lưu
     trong layout mẫu, KHÔNG còn ở QSettings ⚙.)
  - recap_count : số clip (Part) mỗi video. 0 = "Tự động theo độ dài" (MẶC ĐỊNH:
                  <4 phút 1 clip, 4-12 phút 2, 12-20 phút 3, >20 phút 4 —
                  m2_recap tính từ duration); 1-8 = chọn tay — m2_recap chia
                  video thành K chương, mỗi chương 1 clip độc lập Part 1..K.
                  Chọn tay LUÔN được tôn trọng khi video đủ nội dung; video
                  ngắn/ít thoại thì rút về số clip tối đa làm được (báo rõ).
...
class RecapSettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.count.setToolTip(
            "Mỗi lần bấm 🎙 tạo bao nhiêu clip (Part).\n"
            "Tự động theo độ dài (khuyên dùng): video dưới 4 phút = 1 clip, "
            "4-12 phút = 2 clip, 12-20 phút = 3, trên 20 phút = 4.\n"
            "Hoặc chọn tay 1-8. App chia video thành K chương, mỗi chương "
            "1 clip Part riêng (hook + kết riêng).\n"
            "Chọn TAY luôn được tôn trọng khi video ĐỦ nội dung; video quá "
            "ngắn/ít thoại thì app tự rút xuống số clip tối đa làm được và "
            "báo rõ trong hàng đợi.")
        ...
        # (Nhóm "Chữ AI kể" ĐÃ CHUYỂN sang Chỉnh mẫu — phần "Chữ AI đọc". Ở đây
    ...
    def _load(self) -> None:
        ...
        try:                             # số clip: 0 = Tự động (mặc định)
            ...
            cnt = 0
        self.count.setValue(min(8, max(0, cnt)))
