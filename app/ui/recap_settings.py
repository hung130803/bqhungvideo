...
Cài đặt TOÀN CỤC cho mọi kênh (lưu QSettings — không dính vào mẫu template):
  ...
  - recap_volume: "Âm lượng giọng kể" 80-200% (mặc định 115%) — nhân THÊM
                  ...
                  gốc video.
  - recap_win_auto : nhóm "Cắt ghép" — TỰ ĐỘNG để AI tự chọn số cảnh hợp
                  lý theo nội dung (bool, MẶC ĐỊNH BẬT). BẬT -> làm mờ Min/
                  Max, prompt đạo diễn dặn AI tự chọn (thường 2-6), validate
                  dùng bound rộng 2-8. TẮT -> dùng recap_win_min/max.
  - recap_win_min / recap_win_max : nhóm "Cắt ghép" (chỉ khi TẮT tự-động)
                  — số CẢNH (window) AI nên ghép trong 1 clip (Min 2-6, Max
                  3-8, mặc định 3-6, Min<=Max tự ép) -> prompt đạo diễn +
                  trần validate_windows.
  - recap_narr_color / recap_narr_italic / recap_narr_same : nhóm "Chữ AI
                  kể" — chỉnh RIÊNG phụ đề đoạn AI thuyết minh (Style
                  Narrate). color = hex (mặc định #FFD966 vàng), italic =
                  bool (mặc định BẬT), same = bool (mặc định TẮT; BẬT ->
                  đoạn AI kể dùng LUÔN Style Default như phụ đề đoạn gốc).
...
# Màu CHỮ AI KỂ (phụ đề đoạn narrate) — (nhãn, hex lưu QSettings). Vàng #FFD966
# là mặc định cũ (giữ đầu list). Hex khớp Style Narrate trong captions.build_ass.
_NARR_COLORS = [
...
class RecapSettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        note = QLabel(
            ...
            "đoạn GỐC và độ dài Min/Max lấy từ 'Chỉnh mẫu' + 'Tùy chỉnh "
            "cắt'. Ở đây chỉ chỉnh phần GIỌNG + KỊCH BẢN + CHỮ AI kể của "
            "Reup.")
        ...
        # ---- Cắt ghép: số cảnh ghép mỗi clip (auto = AI quyết / chọn tay) ----
        lay.addWidget(QLabel("<b>Cắt ghép</b> — mỗi clip ghép từ bao nhiêu "
                             "cảnh của video gốc"))
        self.win_auto = QCheckBox(
            "Tự động — để AI tự chọn số cảnh hợp lý theo nội dung")
        self.win_auto.setToolTip(
            "BẬT (khuyên dùng): AI TỰ quyết số cảnh ghép cho hợp mạch chuyện "
            "(thường 2-6 cảnh) — không gò cứng.\n"
            "TẮT: bạn ép khoảng Min–Max số cảnh bên dưới (AI trả thừa thì "
            "app cắt bớt, thiếu vẫn nhận).")
        self.win_auto.toggled.connect(self._win_auto_toggled)
        lay.addWidget(self.win_auto)
        wrow = QHBoxLayout()
        self.win_lbl = QLabel("Số cảnh ghép mỗi clip:  Min")
        wrow.addWidget(self.win_lbl)
        ...
        self.win_dash = QLabel("–  Max")
        wrow.addWidget(self.win_dash)
        ...
        # ---- Chữ AI kể: chỉnh RIÊNG phụ đề đoạn AI thuyết minh (narrate) ----
        # Mẫu (Chỉnh mẫu) chỉ lo phụ đề đoạn GỐC (Style Default). Đoạn AI kể
        # dùng Style Narrate riêng — ở đây cho đổi màu / nghiêng / hoặc dùng
        # LUÔN như phụ đề mẫu (không phân biệt).
        lay.addWidget(QLabel("<b>Chữ AI kể</b> — phụ đề riêng cho đoạn AI "
                             "thuyết minh (khác phụ đề đoạn gốc)"))
        nrow = QHBoxLayout()
        nrow.addWidget(QLabel("Màu"))
        self.narr_color = QComboBox()
        for label, hexv in _NARR_COLORS:
            self.narr_color.addItem(label, hexv)
        self.narr_color.setToolTip(
            "Màu chữ phụ đề của đoạn AI KỂ (đoạn gốc giữ màu theo Chỉnh mẫu)."
            "\nĐể người xem phân biệt lời KỂ với lời THOẠI gốc.")
        nrow.addWidget(self.narr_color)
        ...
        nrow.addWidget(self.narr_italic)
        nrow.addStretch(1)
        lay.addLayout(nrow)
        self.narr_same = QCheckBox(
            "Giống hệt phụ đề trong Chỉnh mẫu (không phân biệt đoạn AI kể)")
        self.narr_same.setToolTip(
            "BẬT: đoạn AI kể dùng LUÔN kiểu phụ đề như đoạn gốc (theo Chỉnh "
            "mẫu) — bỏ màu + nghiêng riêng ở trên.\nTẮT (mặc định): đoạn AI "
            "kể có màu + nghiêng riêng để phân biệt.")
        self.narr_same.toggled.connect(self._narr_same_toggled)
        lay.addWidget(self.narr_same)
    ...
    def _load(self) -> None:
        ...
        # Tự-động số cảnh — MẶC ĐỊNH BẬT (làm mờ Min/Max)
        self.win_auto.setChecked(
            str(self._s.value("recap_win_auto", True)).strip().lower()
            not in ("false", "0", "no", "off"))
        self._win_auto_toggled(self.win_auto.isChecked())
        # ---- Chữ AI kể: màu / nghiêng / giống hệt phụ đề mẫu ----
        ncolor = str(self._s.value("recap_narr_color", _NARR_COLOR_DEFAULT)
                     or _NARR_COLOR_DEFAULT)
        ci = self.narr_color.findData(ncolor)
        if ci < 0:                       # màu lưu lạ -> thêm lại giữ lựa chọn
            self.narr_color.addItem(ncolor, ncolor)
            ci = self.narr_color.count() - 1
        self.narr_color.setCurrentIndex(ci)
        self.narr_italic.setChecked(     # in nghiêng — MẶC ĐỊNH BẬT
            str(self._s.value("recap_narr_italic", True)).strip().lower()
            not in ("false", "0", "no", "off"))
        self.narr_same.setChecked(       # giống hệt phụ đề mẫu — MẶC ĐỊNH TẮT
            str(self._s.value("recap_narr_same", False)).strip().lower()
            in ("true", "1", "yes", "on"))
        self._narr_same_toggled(self.narr_same.isChecked())
    ...
    def _save(self) -> None:
        ...
        self._s.setValue("recap_win_auto", bool(self.win_auto.isChecked()))
        # ---- Chữ AI kể ----
        self._s.setValue("recap_narr_color",
                         self.narr_color.currentData() or _NARR_COLOR_DEFAULT)
        self._s.setValue("recap_narr_italic",
                         bool(self.narr_italic.isChecked()))
        self._s.setValue("recap_narr_same", bool(self.narr_same.isChecked()))
        self.accept()
    ...
    def _win_auto_toggled(self, on: bool) -> None:
        """BẬT tự-động -> làm mờ (disable) Min/Max + nhãn (AI tự chọn số
        cảnh); TẮT -> enable chọn tay như cũ."""
        for w in (self.win_lbl, self.win_min, self.win_dash, self.win_max):
            w.setEnabled(not on)

    def _narr_same_toggled(self, on: bool) -> None:
        """BẬT 'giống hệt phụ đề mẫu' -> làm mờ màu + nghiêng riêng (đoạn AI
        kể dùng luôn Style Default); TẮT -> enable chỉnh riêng."""
        self.narr_color.setEnabled(not on)
        self.narr_italic.setEnabled(not on)
