...
Cài đặt TOÀN CỤC cho mọi kênh (lưu QSettings — không dính vào mẫu template):
  ...
  - recap_volume: "Âm lượng giọng kể" 80-200% (mặc định 115%) — nhân THÊM
                  ...
                  gốc video.
  - recap_min_sec / recap_max_sec : nhóm "Độ dài mỗi clip" — mỗi clip Reup
                  dài bao nhiêu GIÂY (Min 10-180, Max 15-600, mặc định 25-80,
                  Min<=Max tự ép). Điều khiển RIÊNG cho Reup (studio_page
                  đưa vào preset min_len/max_len, OVERRIDE 'Tùy chỉnh cắt'
                  chung). Số CẢNH ghép mỗi clip GIỜ LUÔN do AI tự quyết (bỏ
                  hẳn phần "Cắt ghép" — không còn key recap_win_*).
...
class RecapSettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        note = QLabel(
            "Khung 9:16, nền, logo, nhạc nền, hiệu ứng, lật gương và phụ đề "
            "đoạn GỐC lấy từ 'Chỉnh mẫu'. Ở đây chỉnh phần GIỌNG + KỊCH BẢN "
            "+ ĐỘ DÀI + CHỮ AI kể của Reup.")
        ...
        # ---- Độ dài mỗi clip (giây): Min – Max ----
        # Điều khiển RIÊNG cho đường Reup (override 'Tùy chỉnh cắt' chung).
        # Đặt NGAY DƯỚI "Số clip (Part)" để gom nhóm kịch bản/cắt.
        lay.addWidget(QLabel("<b>Độ dài mỗi clip</b> — mỗi clip dài bao nhiêu "
                             "giây"))
        lrow = QHBoxLayout()
        lrow.addWidget(QLabel("Min [giây]"))
        self.min_sec = QSpinBox()
        self.min_sec.setRange(10, 180)
        _len_tip = ("Độ dài mong muốn mỗi clip Reup. AI cố cắt trong khoảng "
                    "này (mong muốn, không cứng tuyệt đối).")
        self.min_sec.setToolTip(_len_tip)
        lrow.addWidget(self.min_sec)
        lrow.addWidget(QLabel("–  Max [giây]"))
        self.max_sec = QSpinBox()
        self.max_sec.setRange(15, 600)
        self.max_sec.setToolTip(_len_tip)
        lrow.addWidget(self.max_sec)
        lrow.addStretch(1)
        # Min <= Max tự ép 2 chiều (tái dùng logic win_min/max cũ).
    ...
    def _load(self) -> None:
        ...
        try:                             # độ dài mỗi clip (giây, mặc định 25-80)
            lmin = int(self._s.value("recap_min_sec", 25))
        except (TypeError, ValueError):
            lmin = 25
        try:
            lmax = int(self._s.value("recap_max_sec", 80))
        except (TypeError, ValueError):
            lmax = 80
    ...
    def _min_sec_changed(self, v: int) -> None:
        ...
        if v > self.max_sec.value():
            self.max_sec.setValue(v)

    def _max_sec_changed(self, v: int) -> None:
        """Hạ Max xuống dưới Min -> Min tụt theo (giữ Min <= Max giây)."""
        if v < self.min_sec.value():
            self.min_sec.setValue(v)
