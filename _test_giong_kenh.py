...
ok("1f giọng CHƯA ĐO -> nhãn RỖNG (không bịa số)",
   nhan_nha.nhan("tr-TR-AhmetNeural") == ""
   and nhan_nha.muc("tr-TR-AhmetNeural") is None)

print("\nCA 2 — luật mở khoá: ĐÃ ĐO THÌ MỞ")
ok("2a mọi giọng edge trong bảng đều được mở",
   all(giong_mo.nen_mo(k) for k in nhan_nha.BANG if ":" not in k))
ok("2b giọng CHƯA ĐO thì KHÔNG mở", not giong_mo.nen_mo("tr-TR-AhmetNeural"))
ok("2c mã KHÔNG phải edge-tts bị từ chối (ov: · piper: · vnb: · cb:)",
   not any(giong_mo.nen_mo(m) for m in
           ("ov:nam_tre", "piper:vi_VN-vais1000-medium", "vnb:a.wav",
            "cb:en|a.wav", "el:Adam", "")))
ok("2d số giọng edge mở ra = 185, phủ 15 thứ tiếng",
   giong_mo.so_giong_mo() == 185 and len(giong_mo.tieng_da_mo()) == 15,
